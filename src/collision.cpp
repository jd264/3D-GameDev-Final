...
bool collision::raysphere(float xc,float yc,float zc,float xd,float yd,float zd,float xs,float ys,float zs,float r,float* dist,vec3D* collpoint)
{
...
float collision::trianglearea(vec3D p1,vec3D p2,vec3D p3)
{
...
float collision::pointdistace(vec3D p1,vec3D p2)
{
	vec3D vec(p2.x-p1.x,p2.y-p1.y,p2.z-p1.z);
	return (sqrt(vec.x*vec.x+vec.y*vec.y+vec.z*vec.z));
}

float collision::pointdistacesquare(vec3D p1,vec3D p2)
{
	vec3D vec(p2.x-p1.x,p2.y-p1.y,p2.z-p1.z);
...
bool collision::rayplane(const float& nx,float ny,float nz,float x0,float y0,float z0,float xs,float ys,float zs,float xd,float yd,float zd,vec3D p1,vec3D p2, vec3D p3,vec3D p4,float* dis,vec3D* point)
{
...
bool collision::sphereplane(vec3D& sp,vec3D pn,vec3D p1,vec3D p2,vec3D p3,vec3D p4, float r)
{
...
bool collision::spheresphere(vec3D& c1,float r1,vec3D& c2,float r2)
{
